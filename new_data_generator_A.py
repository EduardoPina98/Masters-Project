...
for i in range(records):
    ...
    if profile == 'healthy':
        vo2 = np.random.normal(45, 4)
        bmi = np.random.normal(22, 1.5)
        resting_hr = np.random.normal(58, 4)
        spo2 = np.random.normal(97, 0.5)
        sleep_spo2 = np.random.normal(96, 0.6)
        steps = int(np.random.normal(9000, 2000))
        hydration = int(np.random.normal(2000, 300))
        ...
        resting_hr = np.random.normal(66, 6)
        spo2 = np.random.normal(94, 1)
        sleep_spo2 = np.random.normal(93, 1)
        steps = int(np.random.normal(3000, 1000))
        ...
        hydration = int(np.random.normal(1400, 300))
...
for group_name, metrics in metric_groups.items():
    ...
    fig.suptitle(f"{group_name} (Low CVD Risk)", fontsize=14)
    plt.tight_layout(rect=[0, 0, 1, 0.95])

    filename = os.path.join(output_folder, f"{group_name.replace(' ', '_').lower()}_low_risk.png")
...
plt.title("Heatmap - Correlation (Low CVD Risk)", fontsize=20)
plt.tight_layout()

heatmap_file = os.path.join(output_folder, "correlation_heatmap_low_risk.png")
